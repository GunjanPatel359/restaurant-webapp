          ...
          <div>
            ...
            <div className="w-full h-[500px] bg-[url('/food3.jpg')] bg-center bg-cover">
              <div className="bg-coloralpha w-full h-full flex items-center justify-center">
                ...
                <div className="w-full max-w-5xl mx-auto bg-gradient-to-r from-color5 to-color4 p-6 rounded-lg shadow-xl flex flex-col md:flex-row items-center">
                  <div className="flex-1 text-white px-6 md:px-10">
                    <h2 className="text-3xl font-bold mb-4">Welcome to Taste</h2>
                    <p className="text-justify text-md leading-relaxed">
                      Your ultimate destination for seamless dining experiences! Whether you're
                      a food lover, a restaurant owner, or just someone exploring delightful
                      cuisines, our app has you covered. Discover a variety of restaurants,
                      browse mouthwatering menus, and effortlessly place your orders with just
                      a few clicks. Restaurant owners can easily manage menus, track orders,
                      and connect with customers like never before. With Taste, great food is
                      always within your reach—anytime, anywhere. Bon appétit!
                    </p>
                    <div className="mt-6">
                      <button className="bg-green-600 px-6 py-3 text-lg font-semibold rounded-lg border border-green-500 shadow-lg hover:shadow-xl hover:bg-green-700" onClick={()=>router.push('/seller/login')}>
                        Become a Seller
                      </button>
                    </div>
                  </div>
                  <div className="mt-6 md:mt-0 mr-10 flex-shrink-0">
                    ...
                    <div className="flex gap-3 overflow-y-scroll p-5">
                      {hotel.length > 0 &&
                        hotel.map((item, index) => {
                          return (
                            <div
                              className="bg-white min-w-[250px] w-[250px] h-[350px] border border-color1 rounded-2xl shadow-color0 overflow-hidden shadow-lg cursor-pointer"
                              key={index}
                              onClick={() =>
                            ...
                            >
                              <div className="relative w-full h-44 overflow-hidden shadow-md shadow-color0">
                                <img
                                  ...
                                  src={`${SERVER_URL}/uploads/${item.imgUrl}`}
                              ...
                              <div className="p-2 flex flex-col gap-1">
                                ...
                                <div className="flex gap-2">
                                  {item.cusineTypes.map((item, i) => {
                                    return (
                                      <div
                                        className="bg-color0 text-color4 px-2 py-[2px] rounded-xl"
                                        key={i}
                                      >
                                        {item}
                                      </div>
                                ...
                                <div className="text-color5 w-full text-justify">
                                  ...
                                  {`${item.addresses.address}, ${item.addresses.city}, ${item.addresses.state}, ${item.addresses.country}`
                                    .length > 35
                                    ? `${item.addresses.address}, ${item.addresses.city}, ${item.addresses.state}, ${item.addresses.country}`.substring(
                                      0,
                                      35
                                    ) + "..."
                                    : `${item.addresses.address}, ${item.addresses.city}, ${item.addresses.state}, ${item.addresses.country}`}{" "}
                    ...
                    <div className="flex gap-3 overflow-y-scroll p-5">
                      {foodItem.length >= 0 &&
                        foodItem.map((item, index) => {
                          return (
                            <div
                              className="bg-white min-w-[250px] w-[250px] h-[400px] border border-color1 rounded-2xl shadow-color0 overflow-hidden shadow-lg cursor-pointer"
                              key={index}
                              onClick={() =>
                                router.push(`food-item/${item._id}`)
                              }
                            >
